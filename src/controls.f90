MODULE controls
  ...
  ! 'x_dir_gaussian': SIMO feedback control (2D pressure - streamwise periodic Gaussian-type forcing)
  ...
  SUBROUTINE controls_setup_control
    ...
    if (myid .eq. 0) then
      DO WHILE (readinput)
        ...
        INQUIRE(file="./scripts/control_input."//file_num//".inp",exist=readinput)
        ...
        IF (readinput) THEN
          ...
          WRITE(*,*) 'read control input...'
          OPEN(unit=80,file="scripts/control_input."//file_num//".inp",form='formatted',status='old')
          ...
          IF (TRIM(ctrl(n_control)%ctrl_type) .eq. 'x_dir_gaussian') Then
            ...
            WRITE(*,*) 'nx_local=',ctrl(n_control)%actuators(1)%nx_local
            ...
            LLOCATE(ctrl(n_control)%dfdx_local(ctrl(n_control)%actuators(1)%nx_local))
    ...
    DO i=1,n_control
      ...
      IF (TRIM(ctrl(i)%ctrl_type) .eq. 'x_dir_gaussian') THEN
...
! ******************************************************************************
  Subroutine controls_sensing_and_compute_control
    ...
    Do i_ctrl=1,n_control
      IF (TRIM(ctrl(i_ctrl)%ctrl_type) .eq. 'x_dir_gaussian') THEN
...
SUBROUTINE controls_actuating(i_ctrl)
  ...
  IF ( TRIM(ctrl(i_ctrl)%ctrl_type) .eq. 'x_dir_gaussian' ) THEN
    ...
    Do i = 1, ctrl(i_ctrl)%num_act
      ! assign velocity, displacement, normal and tangent1 vector
       if ( istep .eq. 1 .or. ctrl(i_ctrl)%count_u .eq. 1 ) THEN
          if ( istep .eq. 1 ) then
            ! assign displacement
             yb_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%zero_func_local*ctrl(i_ctrl)%y_i_2D(1,i)*amp
            ! assign velocity
             ub_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%zero_func_local*ctrl(i_ctrl)%u_i_2D(1,i)*amp
            ! assign normal vector
             slope_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%dfdx_local*ctrl(i_ctrl)%y_i_2D(1,i)*amp
          else
             yb_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%zero_func_local*ctrl(i_ctrl)%y_i_2D(1000,i)*amp
             ub_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%zero_func_local*ctrl(i_ctrl)%u_i_2D(1000,i)*amp
             ! assign normal vector
              slope_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                      i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                      ctrl(i_ctrl)%dfdx_local*ctrl(i_ctrl)%y_i_2D(1000,i)*amp
          end if

       ELSE
          yb_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                   i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                   ctrl(i_ctrl)%zero_func_local * ctrl(i_ctrl)%y_i_2D(ctrl(i_ctrl)%count_u-1,i) * amp
          ub_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                   i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                   ctrl(i_ctrl)%zero_func_local * ctrl(i_ctrl)%u_i_2D(ctrl(i_ctrl)%count_u-1,i) * amp
          ! assign normal vector
          slope_2D( (i-1)*ctrl(i_ctrl)%actuators(1)%nx_local + 1 : &
                   i   *ctrl(i_ctrl)%actuators(1)%nx_local , : ) = &
                   ctrl(i_ctrl)%dfdx_local*ctrl(i_ctrl)%y_i_2D(ctrl(i_ctrl)%count_u-1,i)*amp
       END IF
