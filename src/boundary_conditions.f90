...
Subroutine apply_boundary_conditions(U_, V_, W_)
    !! Apply boundary conditions to velocity fields in all directions.
    !! Applies ghost cell updates and periodic/Dirichlet boundary conditions to the velocity fields U_, V_, and W_.
    Real   (Int64), CONTIGUOUS, INTENT(INOUT)  :: U_(:, :, :)
        !! X-velocity field (3D array)
    Real   (Int64), CONTIGUOUS, INTENT(INOUT)  :: V_(:, :, :)
        !! Y-velocity field (3D array)
    Real   (Int64), CONTIGUOUS, INTENT(INOUT)  :: W_(:, :, :)
        !! Z-velocity field (3D array)
...
Subroutine apply_periodic_bc_x(F,id)
    ...
    !! Applies periodicity to the field F in the x direction.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field to apply boundary conditions to (3D array)
    Integer(Int32), Intent(In)    :: id
        !! Field type identifier: 1 = x faces, 2 = x centers
...
Subroutine apply_periodic_bc_z(F,id)
    !! Apply periodic boundary conditions in the z direction (with MPI communication).
    !! Communicates boundary planes between MPI processes.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field to apply boundary conditions to (3D array)
    Integer(Int32), Intent(In)    :: id
...
Subroutine apply_Dirichlet_bc_y(F,id)
    !! Apply Dirichlet boundary conditions in the y direction.
    !! For now, only wall boundaries are supported.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field to apply boundary conditions to (3D array)
    Integer(Int32), Intent(In)    :: id
        !! Field type identifier: 1 = y faces, 2 = y centers
...
Subroutine apply_OppControl_bc_y(F,id)
    !! Apply opposition control boundary condition in the y direction.
    !! Only wall-normal velocity is supported.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field to apply boundary conditions to (3D array)
    Integer(Int32), Intent(In)    :: id
        !! Field type identifier: 1 = y faces (should not use 2)
...
Subroutine update_ghost_interior_planes(F,id)
    !! Update ghost interior planes for parallel domain decomposition.
    !! Uses MPI send/recv to update ghost layers between processes.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field to update (3D array)
    Integer(Int32), Intent(In)    :: id
        !! Component identifier (1=U, 2=V, 3=W, 4=other)
...
!-------------------------------------------------------------------------------
!  Send interior velocity planes to update ghost/support layers of neighbors.
!
!  @param[inout] F      Field array (U, V, or W)
!  @param[inout] F_supp Support array to be updated
!  @param[in]    id     Component ID (1=U, 2=V, 3=W)
!  @note Handles periodic boundaries and both directions of communication.
Subroutine interior_planes_update_support(F,F_supp,id)
    !! Send interior velocity planes to update ghost/support layers of neighbors.
    !! Handles periodic boundaries and both directions of communication.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field array (U, V, or W)
    Real   (Int64), Intent(InOut) :: F_supp(:,:,:)
        !! Support array to be updated
    Integer(Int32), Intent(In)    :: id
        !! Component ID (1=U, 2=V, 3=W)
...
!-------------------------------------------------------------------------------
!  Update interior planes using ghost/support planes from neighbors.
!
!  @param[inout] F      Field array (U, V, or W) to be updated
!  @param[inout] F_supp Support/ghost array
!  @param[in]    id     Component ID (1=U, 2=V, 3=W)
!  @note Performs additive update (not overwrite). Assumes periodic domain in z.
Subroutine support_update_interior_planes(F,F_supp,id)
    !! Update interior planes using ghost/support planes from neighbors.
    !! Performs additive update (not overwrite). Assumes periodic domain in z.
    Real   (Int64), Intent(InOut) :: F(:,:,:)
        !! Field array (U, V, or W) to be updated
    Real   (Int64), Intent(InOut) :: F_supp(:,:,:)
        !! Support/ghost array
    Integer(Int32), Intent(In)    :: id
        !! Component ID (1=U, 2=V, 3=W)
  ...
  Elseif (id == 2) Then
    ...
    Call Mpi_sendrecv(buffer_vsupp_s, nxg*ny*(suppz+1), Mpi_real8, sendto, tagto,        &
    buffer_vsupp_r(:,:,1:suppz), nxg*ny*(suppz+1), Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
         istat, ierr)   
    IF (myid==nprocs-1) THEN
  ...
  Elseif (id == 3) Then
    ...
    Call Mpi_sendrecv(buffer_wsupp_s, nxg*nyg*(suppz+1), Mpi_real8, sendto, tagto,        &
    buffer_wsupp_r(:,:,1:suppz), nxg*nyg*(suppz+1), Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
         istat, ierr)   
