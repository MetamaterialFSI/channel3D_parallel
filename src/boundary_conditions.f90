  ...
  Subroutine interior_planes_update_support(F, F_supp, id)
    ...
    Elseif (id == 4) Then
      ...
      If ( myid==0 ) Then 
        recvfrom = nprocs-1
        tagfrom  = 0
        buffer_psupp_s = F(2:nxg-1, 2:nyg-1,nzg-suppz-1:nzg-1) ! send buffer
      ELSEIf ( myid==nprocs-1 ) Then
        sendto = 0
        tagto  = 0
        if (nzg-suppz-2 .lt. 2) Stop 'Error: nzg-suppz-2 less than 2'
        buffer_psupp_s = F(2:nxg-1, 2:nyg-1,nzg-suppz-2:nzg-2) ! send buffer
      ELSE
        buffer_psupp_s = F(2:nxg-1, 2:nyg-1,nzg-suppz-1:nzg-1) ! send buffer
      End If
      Call Mpi_sendrecv(buffer_psupp_s, (nxg-2)*(nyg-2)*(suppz+1), Mpi_real8, sendto, tagto,        &
      buffer_psupp_r, (nxg-2)*(nyg-2)*(suppz+1), Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
           istat, ierr)   
      F_supp(2:nxg-1, 2:nyg-1,1:suppz+1) = buffer_psupp_r ! received buffer
      ...
      If ( myid==0 ) Then
        sendto = nprocs-1
        tagto  = nprocs-1
        buffer_psupp_s(:,:,1:suppz) = F(:,:,3:2+suppz)  ! send buffer
      ELSEIf ( myid==nprocs-1 ) Then
        recvfrom = 0
        tagfrom  = nprocs-1
        buffer_psupp_s(:,:,1:suppz) = F(2:nxg-1, 2:nyg-1,2:1+suppz)  ! send buffer
      ELSE
        buffer_psupp_s(:,:,1:suppz) = F(2:nxg-1, 2:nyg-1,2:1+suppz)  ! send buffer
      End If
        Call Mpi_sendrecv(buffer_psupp_s(:,:,1:suppz), (nxg-2)*(nyg-2)*suppz, Mpi_real8, sendto, tagto,        &
        buffer_psupp_r(:,:,1:suppz), (nxg-2)*(nyg-2)*suppz, Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
             istat, ierr)   
        F_supp(2:nxg-1, 2:nyg-1,suppz+2:2*suppz+1) = buffer_psupp_r(:,:,1:suppz) ! received buffer
  ...
  Subroutine interior_planes_update_support_pressure(F, F_supp)
    ...
    Integer(Int32) :: tagto,  tagfrom
      ...
      If ( myid==0 ) Then 
        recvfrom = nprocs-1
        tagfrom  = 0
        buffer_psupp_s(2:nxg-1,2:nyg-1,1:suppz+1) = F(2:nxg-1, 2:nyg-1,nzg-suppz-1:nzg-1) ! send buffer
      ELSEIf ( myid==nprocs-1 ) Then
        sendto = 0
        tagto  = 0
        if (nzg-suppz-2 .lt. 2) Stop 'Error: nzg-suppz-2 less than 2'
        buffer_psupp_s(2:nxg-1,2:nyg-1,1:suppz+1) = F(2:nxg-1, 2:nyg-1,nzg-suppz-2:nzg-2) ! send buffer
      ELSE
        buffer_psupp_s(2:nxg-1,2:nyg-1,1:suppz+1) = F(2:nxg-1, 2:nyg-1,nzg-suppz-1:nzg-1) ! send buffer
      End If
      Call Mpi_sendrecv(buffer_psupp_s(2:nxg-1,2:nyg-1,1:suppz+1), (nxg-2)*(nyg-2)*(suppz+1), Mpi_real8, sendto, tagto,        &
      buffer_psupp_r(2:nxg-1,2:nyg-1,1:suppz+1), (nxg-2)*(nyg-2)*(suppz+1), Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
           istat, ierr)   
      F_supp(2:nxg-1, 2:nyg-1,1:suppz+1) = buffer_psupp_r(2:nxg-1,2:nyg-1,1:suppz+1) ! received buffer
      ...
      If ( myid==0 ) Then
        sendto = nprocs-1
        tagto  = nprocs-1
        buffer_psupp_s(2:nxg-1, 2:nyg-1,1:suppz) = F(2:nxg-1, 2:nyg-1,3:2+suppz)  ! send buffer
      ELSEIf ( myid==nprocs-1 ) Then
        recvfrom = 0
        tagfrom  = nprocs-1
        buffer_psupp_s(2:nxg-1, 2:nyg-1,1:suppz) = F(2:nxg-1, 2:nyg-1,2:1+suppz)  ! send buffer
      ELSE
        buffer_psupp_s(2:nxg-1, 2:nyg-1,1:suppz) = F(2:nxg-1, 2:nyg-1,2:1+suppz)  ! send buffer
      End If
        Call Mpi_sendrecv(buffer_psupp_s(2:nxg-1, 2:nyg-1,1:suppz), (nxg-2)*(nyg-2)*suppz, Mpi_real8, sendto, tagto,        &
        buffer_psupp_r(2:nxg-1, 2:nyg-1,1:suppz), (nxg-2)*(nyg-2)*suppz, Mpi_real8, recvfrom, tagfrom, MPI_COMM_WORLD, &
             istat, ierr)   
        F_supp(2:nxg-1, 2:nyg-1,suppz+2:2*suppz+1) = buffer_psupp_r(2:nxg-1, 2:nyg-1,1:suppz) ! received buffer

    Call Mpi_barrier(MPI_COMM_WORLD, ierr) 
    
  End Subroutine interior_planes_update_support_pressure
